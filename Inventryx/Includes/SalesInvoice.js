...
function CalculateTotalAmount()
{
    ...
    for(i=2;i<eval(iRow+2);i++)
    {
        ...
        ctrlQtyID='ctl00_mainContent_dgList_ctl0'+i+'_lblQty';
        if(document.getElementById(ctrlQtyID).innerText=='')
        {
            document.getElementById(ctrlQtyID).innerText="0";
        }

        ctrlAmtID='ctl00_mainContent_dgList_ctl0'+i+'_txtAmount';
        
        iRate=SetDecimalPoints(document.getElementById(ctrlRateID).value,2);
        iQty=SetDecimalPoints(document.getElementById(ctrlQtyID).innerText,3);
