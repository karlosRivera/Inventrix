...
public partial class Transactions_JobDetails : BasePage
{
    ...
    string Tables = "vw_TrnJobDetails";
    string PK = "JobNo";
    string Fields = "JobNo,JobStartDate,CustomerName,JobStatus,LabWorkTotal";
    string Filter = "";
    string Group = "";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            Utility.RedirectToLoginPage("JobDetails.aspx");
        }
        ...
        if (!IsPostBack)
        {
            ...
            ReadPageState("JobDetails", out PageNumber, out sSortCol, out sSortOrder);
            if (PageNumber > 1)
            {
                PagerCtrl1.CurrentIndex = PageNumber;
                PagerCtrl2.CurrentIndex = PageNumber;
                BuildSql(PageNumber);
                KillSession("JobDetails");
            }
            else
            {
    ...
    private void BuildSql(int PageNo)
    {
        ...
        DataSet ds = BLL.Transaction.JobDetails.BindGrid(spName, new object[] { Tables, PK, sSortCol, PageNumber, iPageSize, Fields, cFilter, Group, sSortOrder });
    ...
    protected void dgList_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
    {
        ...
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            {
                string sGotoUrl;
                StorePageState("JobDetails", PagerCtrl1.CurrentIndex, sSortCol, sSortOrder);
                sGotoUrl = "JobDetailsView.aspx?Action=C&ID=" + ((Label)e.Item.FindControl("lblJobNo")).Text.Trim();
            ...
            else if (e.CommandName == "Edit")
            {
                string sGotoUrl;
                StorePageState("JobDetails", PagerCtrl1.CurrentIndex, sSortCol, sSortOrder);
                sGotoUrl = "JobDetailsTrn.aspx?Action=E&ID=" + ((Label)e.Item.FindControl("lblJobNo")).Text.Trim();
                Response.Redirect(sGotoUrl);
            }
            else if (e.CommandName == "View")
            {
                string sGotoUrl;
                StorePageState("JobDetails", PagerCtrl1.CurrentIndex, sSortCol, sSortOrder);
                sGotoUrl = "JobDetailsView.aspx?Action=V&ID=" + ((Label)e.Item.FindControl("lblJobNo")).Text.Trim();
                Response.Redirect(sGotoUrl);
            }
