...
public partial class Transactions_ProformaInvoice : BasePage
{
    ...
    string Tables = "vw_TrnProformaInvoice";
    string PK = "ProformaInvNo";
    string Fields = "ProformaInvNo,ProformaInvDate,DeliveryDate,CustomerName,ProformaInvAmt,VatRate,ConfirmationTag,ProformaInvStatus";
    string Filter = "";
    string Group = "";
    string sSortCol = "ProformaInvNo";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            Utility.RedirectToLoginPage("ProformaInvoice.aspx");
        }
        ...
        if (!IsPostBack)
        {
            ...
            Utility.FillFilterDropDown(this.ddSearchField1, "SearchFieldDesc", "SearchFieldID", "SF", "ProformaInvoice");
            Utility.FillFilterDropDown(this.ddSearchCondition1, "FilterDesc", "FilterID", "FC", "ProformaInvoice");
            Utility.FillFilterDropDown(this.ddSearchCondition2, "FilterDesc", "FilterID", "FC", "ProformaInvoice");
            Utility.FillFilterDropDown(this.ddSearchCondition3, "FilterDesc", "FilterID", "FC", "ProformaInvoice");
            Utility.FillFilterDropDown(this.ddSearchField2, "SearchFieldDesc", "SearchFieldID", "SF", "ProformaInvoice");
            Utility.FillFilterDropDown(this.ddSearchField3, "SearchFieldDesc", "SearchFieldID", "SF", "ProformaInvoice");

            ReadPageState("ProformaInvoice", out PageNumber, out sSortCol, out sSortOrder);
            if (PageNumber > 1)
            {
                PagerCtrl1.CurrentIndex = PageNumber;
                PagerCtrl2.CurrentIndex = PageNumber;
                BuildSql(PageNumber);
                KillSession("ProformaInvoice");
            }
            else
            {
    ...
    private void BuildSql(int PageNo)
    {
        ...
        DataSet ds = BLL.Transaction.ProformaInvoice.BindGrid(spName, new object[] { Tables, PK, sSortCol, PageNumber, iPageSize, Fields, cFilter, Group, sSortOrder });
    ...
    protected void dgList_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
    {
        ...
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            {
                string sGotoUrl;
                StorePageState("ProformaInvoice", PagerCtrl1.CurrentIndex, sSortCol, sSortOrder);
                sGotoUrl = "ProformaInvoiceView.aspx?Action=C&ID=" + ((Label)e.Item.FindControl("lblProformaInvNo")).Text.Trim();
            ...
            else if (e.CommandName == "Edit")
            {
                string sGotoUrl;
                StorePageState("ProformaInvoice", PagerCtrl1.CurrentIndex, sSortCol, sSortOrder);
                sGotoUrl = "ProformaInvoiceTrn.aspx?Action=E&ID=" + ((Label)e.Item.FindControl("lblProformaInvNo")).Text.Trim();
                Response.Redirect(sGotoUrl);
            }
            else if (e.CommandName == "View")
            {
                string sGotoUrl;
                StorePageState("ProformaInvoice", PagerCtrl1.CurrentIndex, sSortCol, sSortOrder);
                sGotoUrl = "ProformaInvoiceView.aspx?Action=V&ID=" + ((Label)e.Item.FindControl("lblProformaInvNo")).Text.Trim();
                Response.Redirect(sGotoUrl);
            }
    ...
    protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
    {
        ...
        {
            ((ImageButton)e.Item.FindControl("btnDel")).Attributes.Add("onclick", "javascript:return confirm('Are you sure you want to Cancel this Invoice')");

            string s = (string)(DataBinder.Eval(e.Item.DataItem, "ProformaInvStatus"));

            if ((string)(DataBinder.Eval(e.Item.DataItem, "ProformaInvStatus")) == "CAN")
            {
                ((ImageButton)e.Item.FindControl("btnEdit")).Attributes.Add("onclick","javascript:alert('Sorry Canceled invoice can not be modified');return false;");
            }
            if ((string)(DataBinder.Eval(e.Item.DataItem, "ProformaInvStatus")) == "CAN")
            {
                ((ImageButton)e.Item.FindControl("btnDel")).Attributes.Add("onclick", "javascript:alert('This Invoice is already Canceled');return false;");
            }
        }
    }
    #endregion
