...
public partial class Transactions_JobProductIssue : BasePage
{
    ...
    string Tables = "vw_TrnJobProductIssue";
    string PK = "JobNo";
    string Fields = "JobNo,JobStartDate,JobStatus,CustomerName,ProductAmt,JobStatus";
    string Filter = "";
    string Group = "";
    string sSortCol = "JobNo";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            Utility.RedirectToLoginPage("JobProductIssue.aspx");
        }
        ...
        if (!IsPostBack)
        {
            ...
            Utility.FillFilterDropDown(this.ddSearchField1, "SearchFieldDesc", "SearchFieldID", "SF", "JobDetails");
            Utility.FillFilterDropDown(this.ddSearchCondition1, "FilterDesc", "FilterID", "FC", "JobDetails");
            Utility.FillFilterDropDown(this.ddSearchCondition2, "FilterDesc", "FilterID", "FC", "JobDetails");
            Utility.FillFilterDropDown(this.ddSearchCondition3, "FilterDesc", "FilterID", "FC", "JobDetails");
            Utility.FillFilterDropDown(this.ddSearchField2, "SearchFieldDesc", "SearchFieldID", "SF", "JobDetails");
            Utility.FillFilterDropDown(this.ddSearchField3, "SearchFieldDesc", "SearchFieldID", "SF", "JobDetails");

            ReadPageState("JobProductIssue", out PageNumber, out sSortCol, out sSortOrder);
            if (PageNumber > 1)
            {
                PagerCtrl1.CurrentIndex = PageNumber;
                PagerCtrl2.CurrentIndex = PageNumber;
                BuildSql(PageNumber);
                KillSession("JobProductIssue");
            }
            else
            {
    ...
    private void BuildSql(int PageNo)
    {
        ...
        DataSet ds = BLL.Transaction.JobProductIssueDetails.BindGrid(spName, new object[] { Tables, PK, sSortCol, PageNumber, iPageSize, Fields, cFilter, Group, sSortOrder });
    ...
    protected void dgList_ItemCommand(object source, System.Web.UI.WebControls.DataGridCommandEventArgs e)
    {
        ...
        if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
        {
            ...
            {
                string sGotoUrl;
                StorePageState("JobProductIssue", PagerCtrl1.CurrentIndex, sSortCol, sSortOrder);
                sGotoUrl = "JobProductIssueView.aspx?Action=C&ID=" + ((Label)e.Item.FindControl("lblJobNo")).Text.Trim();
            ...
            else if (e.CommandName == "Edit")
            {
                string sGotoUrl;
                StorePageState("JobProductIssue", PagerCtrl1.CurrentIndex, sSortCol, sSortOrder);
                sGotoUrl = "JobProductIssueTrn.aspx?Action=E&ID=" + ((Label)e.Item.FindControl("lblJobNo")).Text.Trim();
                Response.Redirect(sGotoUrl);
            }
            else if (e.CommandName == "View")
            {
                string sGotoUrl;
                StorePageState("JobProductIssue", PagerCtrl1.CurrentIndex, sSortCol, sSortOrder);
                sGotoUrl = "JobProductIssueView.aspx?Action=V&ID=" + ((Label)e.Item.FindControl("lblJobNo")).Text.Trim();
                Response.Redirect(sGotoUrl);
            }
    ...
    protected void dgList_ItemDataBound(object sender, DataGridItemEventArgs e)
    {
        ...
        {
            ((ImageButton)e.Item.FindControl("btnDel")).Attributes.Add("onclick", "javascript:return confirm('Are you sure you want to Cancel this Job')");

            if ((string)(DataBinder.Eval(e.Item.DataItem, "JobStatus")) == "CAN")
            {
                ((ImageButton)e.Item.FindControl("btnEdit")).Attributes.Add("onclick", "javascript:alert('Sorry Canceled Job can not be modified');return false;");
            }
            if ((string)(DataBinder.Eval(e.Item.DataItem, "JobStatus")) == "CAN")
            {
                ((ImageButton)e.Item.FindControl("btnDel")).Attributes.Add("onclick", "javascript:alert('This Job is already Canceled');return false;");
            }
        }
    }
    # endregion
